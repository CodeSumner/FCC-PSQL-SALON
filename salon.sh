...
MAKE_APPOINTMENT() {
    ...
    CUSTOMERS=$($PSQL "SELECT name FROM customers WHERE phone='$CUSTOMER_PHONE'")
  ...
  if [[ ! $SERVICE_ID_SELECTED =~ ^[0-9]+$ ]]
    then
      ...
      then
          ...
          REGULAR_CUSTOMER=$($PSQL "SELECT customer_id, name FROM customers WHERE phone='$CUSTOMER_PHONE'")
